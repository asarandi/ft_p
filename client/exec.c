...
const char *g_cmds[] = {"!cd", "ls", "cd", "get", "put", "pwd", "quit", "help"};

static int (*g_functions[])(t_ftp *) = {&cmd_cd_local, &cmd_ls,  &cmd_cd,
                                        &cmd_get,      &cmd_put, &cmd_pwd,
                                        &cmd_quit,     &cmd_help};
...
int client_send_raw(t_ftp *f) {
    ...
    cmd = str_join(&f->input[1], "\r\n");
    write(f->socket, cmd, strlen(cmd));
    free(cmd);
    return (1);
}
...
int client_execute_command(t_ftp *f) {
    ...
    while (i < CMD_COUNT) {
        if ((word(f->input, 0) != NULL) &&
            (strncmp(word(f->input, 0), g_cmds[i], strlen(g_cmds[i])) == 0))
